...
sorting(int level, int front, struct proc* queue[])
{
  ...
  // if(level == 0){
    for(i=1; i < front; i++){
      if(queue[i] != 0) count += 1;
      
      p = queue[i];
      for(j=i-1; j>=0 && (queue[j]->real_tick > p->real_tick || queue[j] == 0); j--){ //dequeue => 0
        queue[j+1] = queue[j];
      }
      queue[j+1] = p;
    }
  ...
  //     if(L1[i] != 0) count++;
  //     p = L1[i];
  //     for(j=i-1; j>=0 && (L1[j]->proc_tick > p->proc_tick || L1[j] == 0); j--){ //dequeue => 0
  //       L1[j+1] = L1[j];
  //     }
  //     L1[j+1] = p;
  //   }
  ...
  //     if(L2[i] != 0) count++;
  //     p = L2[i];
  //     for(j=i-1; j>=0 && (L2[j]->priority > p->priority); j--){ //dequeue => 0
  //       L2[j+1] = L2[j];
  //     }
  //     L2[j+1] = p;
  //   }
...
// void
// Enqueue(int level, int* front, struct proc* p)
// {
//   if(level == 0){
//     L0[*front] = p;
//     *front = *front+1;
