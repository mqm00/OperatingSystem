...
for (;;) {
    ...
    for (int i = 0; i < NPROC; i++) { 
      ...
      if(ptable.proc[i].flag_Lock == 1){ //schedulerLock
      p = &ptable.proc[i];
      break;
      }
      if(ptable.proc[i].flag_Lock == 2){ //schedulerUnlock 
        ptable.proc[i].priority = 3;
        p = &ptable.proc[i];
        if(p->proc_tick == 4) p->flag_Lock = 0; //L0의 처음에서 L1으로 이동하게 될 때 flag를 0으로 바꿈으로써 다른 
        break;
      }
      
      if (ptable.proc[i].cur_queue == 0) {  // L0
            p = &ptable.proc[i];
            break;
      }
    }
